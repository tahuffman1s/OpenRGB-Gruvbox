...
OpenRGBDialog2::OpenRGBDialog2(QWidget *parent) : QMainWindow(parent), ui(new OpenRGBDialog2Ui)
{
    ...
    if (load_geometry)
    {
        ...
        \*-----------------------------------------------------*/
        if (ui_settings["geometry"].contains("x") && ui_settings["geometry"].contains("y"))
        {
            set_window.setX(ui_settings["geometry"]["x"].get<int>());
            set_window.setY(ui_settings["geometry"]["y"].get<int>());
        }

        if (ui_settings["geometry"].contains("width") && ui_settings["geometry"].contains("height"))
        {
    ...
    if (ui_settings.contains("greyscale_tray_icon"))
    {
        if (ui_settings["greyscale_tray_icon"].get<bool>())
        {
            SetTrayIcon("greyscale");
        }
        else
        {
    ...
    /*-------------------------------------------------*\
    | If Gruvbox Tray Icon flag is not set in config    |
    | then set the default value to false               |
    \*-------------------------------------------------*/
    if (!ui_settings.contains("gruvbox_tray_icon"))
    {
        ui_settings["gruvbox_tray_icon"] = false;
    ...
    /*-----------------------------------------------------*\
    | If Gruvbox tray icon exists in settings, apply it     |
    |   or else set the icon to the default window logo     |
    \*-----------------------------------------------------*/
    if (ui_settings.contains("gruvbox_tray_icon"))
    {
        if (ui_settings["gruvbox_tray_icon"].get<bool>())
        {
            SetTrayIcon("gruvbox");
        }
        else
...
OpenRGBDialog2::~OpenRGBDialog2()
{
    ...
    if (ui_settings.contains("geometry"))
    {
        ...
        {
            if (ui_settings["geometry"]["load_geometry"].get<bool>() && ui_settings["geometry"]["save_on_exit"].get<bool>())
            {
...
void OpenRGBDialog2::LoadExitProfile()
{
    ...
    if (ui_settings.contains(exit_profile))
    {
        if (ui_settings[exit_profile].contains(set_on_exit) && ui_settings[exit_profile].contains(profile_name))
        {
...
void OpenRGBDialog2::AddSettingsPage()
{
    ...
    \*-----------------------------------------------------*/
    connect(SettingsPage, SIGNAL(TrayIconChanged(std::string)), this, SLOT(SetTrayIcon(std::string)));
...
void OpenRGBDialog2::SetTrayIcon(std::string tray_icon)
{
    if (tray_icon == "greyscale")
    {
        trayIcon->setIcon(QIcon(":OpenRGBGreyscale.png"));
    }
    else if (tray_icon == "gruvbox")
    {
        trayIcon->setIcon(QIcon(":OpenRGBGruvbox.png"));
    }
    else
    {
